...
2. 筛选出中文字数 < 10万字的文件
3. 按文件名排序，将多个文件合并
4. 确保合并后的文件中文字数不超过 20万字
5. 保存到 files/combined/ 目录
6. 可选：删除原文件（带备份）
"""
...
def natural_sort_key(path: Path) -> list:
    ...
    例如：file1.txt, file2.txt, file10.txt 会按数字大小排序
    而不是字符串排序（file1, file10, file2）
...
def merge_entrance(
    ...
    Args:
        files_dir: 输入文件目录（默认 "files"）
        delete_originals: 是否删除原文件（默认 False）
    ...
    if delete_originals:
        logger.info("\n步骤 4/4: 删除原文件")
        delete_results = delete_original_files(
            file_list=filtered_files,
            backup=backup
        )
    else:
